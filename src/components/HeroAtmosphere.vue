...
const fragment=`precision highp float;varying vec2 uv;uniform vec2 resolution;uniform float time;uniform vec2 eyeLeft;uniform vec2 eyeRight;uniform vec2 eyeSize;uniform vec2 cupPos;uniform vec2 cupSize;
...
vec2 steamUV=(uv-cupPos)/cupSize;
float steamMask=smoothstep(0.,.12,steamUV.y)*(1.-smoothstep(1.15,1.65,steamUV.y));
float wisp1=exp(-abs(steamUV.x+sin(steamUV.y*5.2-time*1.15)*.24)*9.5);
float wisp2=exp(-abs(steamUV.x-.32+sin(steamUV.y*4.1-time*.9+1.8)*.18)*12.);
float steam=(wisp1*.18+wisp2*.11)*steamMask*(.72+.28*sin(time*1.4+steamUV.y*7.));
color+=vec3(.76,.73,.66)*steam;
alpha=max(alpha,steam*.62);
